...
CREATE TYPE request_status AS ENUM ('draft', 'submitted', 'approved', 'in_progress', 'review_pending', 'review_approved', 'review_rejected', 'completed', 'cancelled');
...
CREATE TABLE IF NOT EXISTS restoration_requests (
    ...
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    CONSTRAINT check_precious_review CHECK (
        CASE 
            WHEN (SELECT rarity_level FROM ancient_books WHERE id = book_id) IN ('precious', 'national_treasure') 
            THEN status IN ('review_pending', 'review_approved', 'review_rejected') OR status <= 'in_progress'
            ELSE true
        END
    )
);
...
CREATE TABLE IF NOT EXISTS restoration_steps (
    ...
    CONSTRAINT check_material_batch CHECK (
        CASE 
            WHEN status = 'completed' AND material_id IS NOT NULL
            THEN (SELECT batch_number FROM materials WHERE id = material_id) IS NOT NULL
            ELSE true
        END
    )
);
...
BEGIN
    IF NEW.status = 'completed' THEN
        ...
        SET current_status = CASE 
            WHEN has_approved_review THEN 'available'
            ELSE 'restricted'
        END
    ...
    IF NEW.status = 'in_progress' THEN
        UPDATE ancient_books 
        SET current_status = 'under_restoration'
        WHERE id = NEW.book_id;
    END IF;
...
BEGIN
    IF NEW.status = 'completed' AND NEW.material_id IS NOT NULL THEN
        SELECT batch_number INTO material_batch FROM materials WHERE id = NEW.material_id;
        IF material_batch IS NULL OR material_batch = '' THEN
